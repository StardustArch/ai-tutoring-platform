...
async function handleGoogleLogin() {
    ...
    try {
      // 1. Fazemos um "ping" à rota de saúde do teu backend.
      // Substitui '/api/health' pela rota que criaste ou qualquer rota leve e pública do teu NestJS.
      // Se não criaste nenhuma, tentar fazer GET à raiz da API também costuma funcionar se retornar algo.
      const pingResponse = await fetch(`${PUBLIC_API_URL_HOST}/api/health/wakeup`);
      
      if (pingResponse.ok) {
        // 2. O Backend está acordado! Agora sim, podemos redirecionar com segurança.
        console.log("☀️ Backend pronto! A iniciar fluxo do Google...");
        window.location.href = `${PUBLIC_API_URL_HOST}/api/auth/google`;
      } else {
         throw new Error("O servidor demorou muito a responder.");
      }
    } catch (err) {
      console.error("❌ Falha ao contactar o servidor:", err);
      error = "Os nossos servidores estão a aquecer. Por favor, aguarde 30 segundos e tente novamente.";
...
<button 
        ...
        class="w-full flex items-center justify-center gap-3 px-4 py-3 bg-white dark:bg-surface-800 border border-surface-200 dark:border-surface-700 rounded-lg hover:bg-surface-50 dark:hover:bg-surface-700 transition-all text-sm font-bold text-surface-700 dark:text-white group disabled:opacity-70 disabled:cursor-not-allowed"
      >
        {#if isGoogleLoading}
          <Loader2 size={18} class="animate-spin text-surface-500" />
          <span>A preparar ligação...</span>
        {:else}
