<script lang="ts">
  ...
  import {
    ArrowLeft, CheckCircle2, X, Star, BookOpen, PenLine
  } from 'lucide-svelte';
  ...
  async function startLesson() {
    ...
    try {
      const res = await apiFetch(`${PUBLIC_API_URL_HOST}/api/lesson/start`, {
        ...
        body: JSON.stringify({ alunoId: parseInt(studentId), topicoId, turmaId: turmaId || undefined })
      });
  ...
  async function handleAnswer(option: string) {
    ...
    if (isCorrect) {
      ...
      inputShake = true;
      setTimeout(() => { inputShake = false; }, 500);
    }
    ...
    try {
      ...
      screen       = 'feedback';
      if (data.done) {
        setTimeout(() => { confetti({ particleCount: 200, spread: 100 }); screen = 'done'; }, 1400);
      }
  ...
  async function continuar() {
    if (revisaoCount > 0 && fase === 'normal') { screen = 'revisao_intro'; return; }
    await pedirProxima();
  }
  ...
  function sair() { goto(`/dashboard/foreman/student/${studentId}/class`); }

  // ─── Reactividade ─────────────────────────────────────────────────────────
  $: progressPercent = totalSlots > 0 ? Math.round((slotIndex / totalSlots) * 100) : 0;
...
<div class="flex h-[100dvh] flex-col overflow-hidden bg-gradient-to-b from-sky-100 via-blue-50 to-white font-['Fredoka']">
  ...
  <div class="z-10 flex shrink-0 items-center gap-3 border-b-4 border-sky-200 bg-white/90 p-3 shadow-sm backdrop-blur-sm">
    <button on:click={sair} class="shrink-0 rounded-xl border-2 border-slate-200 bg-white p-2 text-slate-400 shadow-sm transition-all hover:border-sky-400 hover:text-sky-500 active:scale-95">
    ...
    <div class="flex flex-1 flex-col gap-1">
      ...
      <div class="h-3 w-full overflow-hidden rounded-full bg-slate-100">
        <div class="h-full rounded-full transition-all duration-500 {fase === 'revisao' ? 'bg-amber-400' : 'bg-sky-500'}" style="width:{progressPercent}%"></div>
      </div>
    </div>
  ...
  {:else if screen === 'question' || screen === 'feedback'}
    <div class="scrollbar-hide mx-auto flex w-full max-w-2xl flex-1 flex-col justify-start overflow-y-auto p-4 pb-40 md:p-6">
      ...
      {#if questionType === 'true_false'}
        <div class="mb-3 flex justify-center">
          <span class="rounded-full border-2 border-violet-200 bg-violet-100 px-3 py-1 text-xs font-black text-violet-700">✅❌ Verdadeiro ou Falso</span>
        </div>
      {:else if questionType === 'direct_input'}
        <div class="mb-3 flex justify-center">
          <span class="flex items-center gap-1 rounded-full border-2 border-sky-200 bg-sky-100 px-3 py-1 text-xs font-black text-sky-700">
          ...
          <h1 class="text-center text-xl leading-snug font-black text-slate-800 md:text-2xl">
            {#each clozeParts as part, i}
              {part}{#if i < clozeParts.length - 1}
                {#if selectedOption}
                  <span class="mx-1 inline-block rounded-lg px-2 py-0.5 {isCorrect ? 'bg-green-100 text-green-700' : 'bg-rose-100 text-rose-700'}">{selectedOption}</span>
                {:else}
                  <span class="mx-1 inline-block min-w-[60px] border-b-4 border-teal-400 text-center text-teal-400">___</span>
                {/if}
              {/if}
            {/each}
          </h1>
        {:else}
          <h1 class="text-center text-xl leading-snug font-black text-slate-800 md:text-2xl">{question}</h1>
        {/if}
      </div>
          ...
          {#each ['Verdadeiro', 'Falso'] as opt}
            ...
            <button
              ...
              class="flex min-h-[90px] flex-col items-center justify-center gap-2 rounded-3xl border-b-4 p-5 text-lg font-black transition-all
                {right || show ? 'scale-[1.02] border-green-700 bg-green-500 text-white'
                : wrong        ? 'border-rose-700 bg-rose-500 text-white'
                : opt === 'Verdadeiro' ? 'border-emerald-300 bg-emerald-50 text-emerald-700 active:translate-y-1 active:border-b-0'
      ...
      {:else if questionType === 'direct_input'}
        <div class="flex flex-col items-center gap-4">
          <input
            ...
            class="w-full max-w-sm rounded-2xl border-b-4 bg-white p-4 text-center text-xl font-black text-slate-800 shadow-md outline-none transition-all placeholder:text-slate-300 focus:ring-2 focus:ring-sky-200 disabled:opacity-70
          ...
          {#if !selectedOption}
            ...
            >CONFIRMAR ✓</button>
          {:else if !isCorrect}
            <div class="w-full max-w-sm rounded-2xl border-2 border-green-200 bg-green-50 p-3 text-center">
              <p class="mb-1 text-xs font-bold uppercase text-green-600">Resposta correta</p>
        ...
        <div class="grid w-full grid-cols-1 gap-3">
          {#each options as opt}
            <button
              on:click={() => handleAnswer(opt)}
              disabled={!!selectedOption}
              style={selectedOption && opt !== selectedOption && opt !== correctAnswer ? 'opacity:0.5' : ''}
              class="group relative flex min-h-[60px] items-center justify-between rounded-2xl border-b-4 p-4 text-left font-bold transition-all
                {selectedOption === opt && isCorrect  ? 'scale-[1.01] border-green-700 bg-green-500 text-white'
                : selectedOption === opt && !isCorrect ? 'border-rose-700 bg-rose-500 text-white'
                : selectedOption && opt === correctAnswer ? 'border-green-700 bg-green-500 text-white'
    ...
    <!-- Painel de feedback ────────────────────────────────────────────────── -->
    {#if screen === 'feedback'}
      <div class="animate-slide-up fixed inset-x-0 bottom-0 z-50 rounded-t-3xl p-4 shadow-[0_-10px_50px_rgba(0,0,0,0.2)] md:p-6 {isCorrect ? 'border-t-8 border-green-500 bg-green-100' : 'border-t-8 border-rose-500 bg-rose-100'}">
        <div class="mx-auto flex max-w-3xl flex-col items-center justify-between gap-4 sm:flex-row">
          <div class="w-full flex-1 text-center sm:text-left">
            {#if isCorrect}
              ...
              <p class="text-sm font-medium leading-relaxed text-green-800">{explanation}</p>
            {:else}
          ...
          <button
            on:click={continuar}
            class="w-full shrink-0 rounded-2xl border-b-4 px-8 py-4 text-lg font-black text-white shadow-xl active:translate-y-1 active:border-b-0 sm:w-auto {isCorrect ? 'border-green-700 bg-green-500' : 'border-rose-700 bg-rose-500'}"
          >CONTINUAR</button>
        </div>
  ...
  {:else if screen === 'revisao_intro'}
    <div class="animate-zoom-in flex flex-1 flex-col items-center justify-center p-6 text-center">
      ...
      <p class="mx-auto mb-8 max-w-xs text-sm text-slate-500">Vamos repeti-las agora para completares a lição.</p>
      <button on:click={pedirProxima} class="w-full max-w-xs rounded-2xl border-b-4 border-amber-600 bg-amber-400 px-8 py-4 text-lg font-black text-white shadow-lg active:translate-y-1 active:border-b-0">VAMOS LÁ! 💪</button>
    </div>
    ...
    <div class="animate-zoom-in flex flex-1 flex-col items-center justify-center p-6 text-center">
      ...
      <div class="mb-8 w-full max-w-sm rounded-3xl border-2 border-slate-100 bg-white p-6 shadow-lg">
        <div class="grid grid-cols-3 gap-4 text-center">
          <div><p class="text-3xl font-black text-amber-500">{xpGanho}</p><p class="text-xs font-bold uppercase text-slate-400">XP</p></div>
          <div><p class="text-3xl font-black text-green-500">{acertos}</p><p class="text-xs font-bold uppercase text-slate-400">Certas</p></div>
          <div><p class="text-3xl font-black text-slate-600">{respostas}</p><p class="text-xs font-bold uppercase text-slate-400">Total</p></div>
        </div>
      </div>
      <button on:click={sair} class="w-full max-w-xs rounded-2xl border-b-4 border-sky-700 bg-sky-500 px-8 py-4 text-lg font-bold text-white shadow-lg active:translate-y-1 active:border-b-0">Voltar ao Menu</button>
    </div>
  {/if}
</div>
