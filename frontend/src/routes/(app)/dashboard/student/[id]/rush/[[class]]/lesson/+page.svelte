<script lang="ts">
    ...
    import { ArrowLeft, CheckCircle2, X, Star, BookOpen, PenLine, Image as ImageIcon, FileText } from 'lucide-svelte';
    ...
    // 🆕 ÂNCORAS (Contexto Visual/Textual)
...
ancoraTipo    = data.ancora?.tipo     || null;
        ...
        // Se o slot tiver âncora, mostramos o modal automaticamente para a criança ver logo o contexto
        if (ancoraKey) {
    ...
    $: isVisualAnchor = ancoraKey?.startsWith('visual_');
</script>
...
{#if showAncoraModal && ancoraKey}
    ...
    <div 
        ...
        on:click={() => { showAncoraModal = false; if(questionType === 'direct_input') inputEl?.focus(); }}
    >
        <div 
        ...
        >
            <div class="flex items-center justify-between border-b-4 border-slate-100 bg-slate-50 p-4">
                <h2 class="flex items-center gap-2 text-lg font-black text-slate-700">
                    {#if isVisualAnchor}
                        <ImageIcon class="text-sky-500" size={24} /> Observa a Imagem
                    {:else}
            ...
            <div class="overflow-y-auto p-4 sm:p-6">
                {#if isVisualAnchor}
                    <img src={`/ancoras/${ancoraKey}.jpg`} alt="Contexto Visual" class="w-full rounded-2xl object-contain shadow-sm" />
                {:else}
                    <div class="rounded-2xl border-2 border-amber-100 bg-amber-50 p-6 font-medium text-slate-800 leading-relaxed">
                        <p>Conteúdo do texto: {ancoraKey}</p>
                    </div>
                {/if}
            </div>
            ...
            <div class="border-t-4 border-slate-100 p-4">
                <button 
                    on:click={() => { showAncoraModal = false; if(questionType === 'direct_input') inputEl?.focus(); }}
                    class="w-full rounded-2xl border-b-4 border-sky-700 bg-sky-500 py-4 text-lg font-black text-white shadow-md active:translate-y-1 active:border-b-0"
                >
            ...
            {#if ancoraKey}
                <div class="mb-4 flex justify-center">
                    <button 
                        ...
                        class="flex animate-pulse items-center gap-2 rounded-full border-b-4 border-amber-600 bg-amber-400 px-5 py-2 font-black text-white shadow-md active:translate-y-1 active:border-b-0"
                    >
                        {#if isVisualAnchor}
...
<style>
    /* Todas as tuas animações e estilos originais mantidos */
    @keyframes fade-in {
        from { opacity: 0; }
        to { opacity: 1; }
    }
