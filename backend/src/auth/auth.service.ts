...
export class AuthService {
  ...
  >();
  generateOAuthCode(tokens: { accessToken: string; refreshToken: string }) {
  ...
  async confirmEmail(token: string) {
    try {
      ...
      this.mailService
        .sendWelcome(user.email, user.nome)
        .catch((e) => console.error(e));
  ...
  async validateOAuthUser(payload: OAuthPayload): Promise<Usuario> {
    ...
    this.mailService
      .sendWelcome(newUser.email, newUser.nome)
      .catch((e) => console.error(e));
  ...
  async forgotPassword(dto: ForgotPasswordDto) {
    ...
    if (!user) {
      ...
      return {
        message: 'Se o email existir, receberá um link de recuperação4.',
      };
    }
  ...
  async changePassword(userId: number, dto: ChangePasswordDto) {
    ...
    if (!user || !user.passwordHash)
      throw new NotFoundException('Utilizador inválido.');
